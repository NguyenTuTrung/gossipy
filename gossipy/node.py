...
class ChordNode(GossipNode):
    ...
    def send(self,
             ...
             sender: int,
             peer: int,
             protocol: AntiEntropyProtocol) -> Union[ChordMessage, None]:

        if protocol == AntiEntropyProtocol.PUSH:
            key = self.model_handler.caching(sender)
            return ChordMessage(t, sender, peer, MessageType.PUSH, (key,))
        elif protocol == AntiEntropyProtocol.PULL:
            return ChordMessage(t, sender, peer, MessageType.PULL, None)
        elif protocol == AntiEntropyProtocol.PUSH_PULL:
            key = self.model_handler.caching(sender)
            return ChordMessage(t, sender, peer, MessageType.PUSH_PULL, (key,))
        else:
            raise ValueError("Unknown protocol %s." %protocol)
    ...
    def receive(self, t: int, msg: ChordMessage) -> Union[ChordMessage, None]:
        ...
        if  msg_type == MessageType.PULL or \
            msg_type == MessageType.PUSH_PULL:
            key = self.model_handler.caching(sender)
            return ChordMessage(t, self.idx, sender, MessageType.REPLY, (key,))
        return None

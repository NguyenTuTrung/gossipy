...
class ChordSimulator(GossipSimulator):
    ...
    def build_chord_tree(self, idx: int) -> Dict[int, ChordNode]:
        used_chord = set()
        queue = [idx]
        ...
        while queue:
            i = queue.pop(0)
            used_chord.add(idx)
            for finger_idx in self.nodes[i].finger.copy():
                if finger_idx not in used_chord:
                    queue.append(finger_idx)
                    used_chord.add(finger_idx)
                else:
                    del self.nodes[i].finger[self.nodes[i].finger.index(finger_idx)]
        ...
        try:
            for t in pbar:
                if t % self.delta == 0: 
                    shuffle(node_ids)
                for i in node_ids:
                    self.nodes = self.build_chord_tree(i)
                    queue = [i]
                    while queue:
                        node_idx = queue.pop(0)
                        if self.nodes[node_idx].timed_out(t, W_matrix[node_idx]):
                            # print('dang chay')
                            for finger_idx in self.nodes[node_idx].finger:
                                msg = self.nodes[node_idx].send(t, node_idx, finger_idx, self.protocol)
                                queue.append(finger_idx)
                                self.notify_message(False, msg)
                                msg_cnt += 1
                                ...
                                if msg:
                                    if random() >= self.drop_prob:
                                        ...
                                        self.notify_message(True)
                        # else:
                        #     print('chua het thoi gian' + '.....' + str(self.nodes[i].idx))
